    ...
    if is_dir:
        ...
        for f in os.listdir(src_folder):
            if f.endswith('.json'):
                ...
                dst_path = os.path.join(dst_folder, "noise_blur_low.json")
...
def upload_dataset_open(dataset_folder, dataset_name, is_video, unzip_file=True,sub_dir=None):
    ...
    if sub_dir is not None:
        train_dataset_dst_folder=train_dataset_dst_folder+sub_dir
    train_dataset_path = train_dataset_dst_folder
    if is_video:
        train_dataset_path += f'/{dataset_name}/'

    if not unzip_file:
        return train_dataset_path,new_root_dst_folder
...
def upload_dataset(dataset_root_folder, dataset_name, unzip_file=True, new_root_dst_folder=None):
    
    for dataset in dataset_name:
        train_dataset_src_folder = f'{dataset_root_folder}/{dataset}'
         
        if not unzip_file:
            return dataset_root_folder,dataset_root_folder
        if not new_root_dst_folder:
            new_root_dst_folder=get_dataset_dst_folder()
        os.system('df')
    ...
    return f"{new_root_dst_folder}dataset/train", f"{new_root_dst_folder}dataset/train"
