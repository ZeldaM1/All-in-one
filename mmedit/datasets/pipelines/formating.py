...
from mmedit.utils import rgb2ycbcr
...
@PIPELINES.register_module()
class ImageToTensor_trans(ImageToTensor):
    def __init__(self, keys, to_float32=True,convert_to=None,channel_order=None):
        super(ImageToTensor_trans, self).__init__(keys=keys,to_float32=to_float32)
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            ...
            if self.convert_to is not None:
                assert self.convert_to=='yuv'
                assert self.channel_order == 'rgb'
                results[key]=rgb2ycbcr(results[key])
...
class MergeLQ:
    def __call__(self, results):
        results_new=dict()
        results_new['lq']=(results['lq'], results['lq2'])
        results_new['gt']=results['gt']
        return results_new

@PIPELINES.register_module()
class ImageToTensor_trans_patch(ImageToTensor_trans):
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            ...
            if len(results[key][0].shape) == 2:
                results[key][0], results[key][1] = results[key][0][..., None], results[key][1][..., None]
            if self.to_float32 and not isinstance(results[key][0], np.float32):
                results[key][0], results[key][1] = results[key][0].astype(np.float32), results[key][1].astype(np.float32)

            results[key][0], results[key][1] = to_tensor(results[key][0].transpose(2, 0, 1)), to_tensor(results[key][1].transpose(2, 0, 1))
            
            if self.convert_to is not None:
                assert self.convert_to=='yuv'
                assert self.channel_order == 'rgb'
                results[key][0],results[key][1]=rgb2ycbcr(results[key][0]), rgb2ycbcr(results[key][1])
