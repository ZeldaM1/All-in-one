...
class Gaussian_Noise:
    ...
    def _apply_gaussian_noise(self, imgs):
        outputs = []
        for img in imgs:
            degrade_type = random.randint(0, len(self.params['gaussian_sigma'])-1)
            sigma= self.params['gaussian_sigma'][degrade_type] / 255.
            # print('---------',img.min(),img.max(),self.params['gaussian_sigma'][degrade_type],sigma)
            is_gray_noise = np.random.uniform() < self.params['gaussian_gray_noise_prob']
...
@PIPELINES.register_module()
class SyntheticRandomNoise:
 
    def __init__(self, keys, noise_level=1):
        self.keys = keys
    ...
    def _apply_read_shot_noise(self, img):
 
        # degamma
        noisy_degamma = UndosRGBGamma(img)
        # read and shot noise
    ...
    def __call__(self, results):
        for key in self.keys:
            results[key] = self._apply_read_shot_noise(results[key])
...
@PIPELINES.register_module()
class Gaussian_Noise_test:
 
    def __init__(self, params, keys, degrade_type=0, is_gray_noise=False, seed=0):
        self.keys = keys
        self.params = params
        self.seed=seed
    ...
    def _apply_gaussian_noise(self, img):
        sigma= self.degrade_type/255. #self.params['gaussian_sigma'][self.degrade_type] / 255.
        # print('---------',img.min(),img.max(),self.params['gaussian_sigma'][degrade_type],sigma)
...
@PIPELINES.register_module()
class DegradationsRandomChoose:
    def __init__(self, degradations=[], data_probs=[0.3,0.2,0.5], probs=[2, 1], src_keys=['gt'], dst_keys=['lq']): 
        self.data_probs=[i/sum(data_probs) for i in data_probs]
        self.probs=[i/sum(probs) for i in probs]
        
        self.src_keys=src_keys
        self.dst_keys=dst_keys
        self.degradations = self._build_degradations(degradations)
        assert len(self.degradations)==len(probs)
    ...
    def _build_degradations(self, degradations):
        for i, degradation in enumerate(degradations):
            if isinstance(degradation, (list, tuple)):
                ...
                degradation_ = allowed_degradations[degradation['type']]
                degradations[i] = degradation_(degradation['params'], degradation['keys'])

        return degradations

    def __call__(self, results):
        data_type=np.random.choice([0,1,2], p=self.data_probs)
        
        if data_type==2: # rain/snow/hazy/low-light
            return results

        # clean/blur/noise, lq is generated based on gt, lq=gt
        for (src_key, dst_key) in zip(self.src_keys, self.dst_keys):
            results[dst_key] = copy.deepcopy(results[src_key])  
 
        if data_type==0: # clean
            return results
        elif data_type==1: # blur/noise
            degradation = np.random.choice(self.degradations, p=self.probs)
            results = degradation(results)
            return results
    ...
    def __repr__(self):
        ...
        repr_str += (f'(dst_keys={self.degradations})')
        return repr_str


@PIPELINES.register_module()
class DegradationsRandomChoosev2(DegradationsRandomChoose):
 

    def __call__(self, results):
        if 'LOL' in results['lq_path']: # rain/snow/hazy/low-light
            return results

        data_type=np.random.choice([0,1], p=self.data_probs)
        # clean/blur/noise, lq is generated based on gt, lq=gt
        for (src_key, dst_key) in zip(self.src_keys, self.dst_keys):
            results[dst_key] = copy.deepcopy(results[src_key])  
