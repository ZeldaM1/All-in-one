...
@PIPELINES.register_module()
class Resize_min(Resize):
    def __init__(self, keys, min_size=None, **kwargs):
        super().__init__(keys=keys, **kwargs)
        self.min_size=min_size
 
    def __call__(self, results):
        h, w = results[self.keys[0]].shape[:2] 
        if self.min_size and ((h<self.min_size) or (w<self.min_size)):

            if h<=w:
                new_h = self.min_size
                new_w = int(self.min_size / h * w)
            elif w<h:
                new_w = self.min_size
                new_h = int(self.min_size / w * h)
...
def data_augmentation(aug_mode):
    def mode0(image):
        return image
    def mode1(image):
        # flip up and down
    ...
    return locals()[f'mode{aug_mode}']

@PIPELINES.register_module()
class AirNetaugmentation:
    ...
    def __call__(self, results):
        ...
        aug1 = data_augmentation(random.randint(1, 7))
        aug2 = data_augmentation(random.randint(1, 7))
        
 
        for key in self.keys:
            results[key]=[aug1(results[key]),aug2(results[key])]
        return results
            ...
            if isinstance(results[key], list):
                for v in results[key]:
                    ...
                    if v.shape[0]>v.shape[1]:
                        key_each.append(v.transpose(1, 0, 2))
                    else:
                        key_each.append(v)
               
            else:
                if results[key].shape[0]>results[key].shape[1]:
                    results[key] = results[key].transpose(1, 0, 2)
...
class CopyValues:
    ...
    def __init__(self, src_keys, dst_keys, copy_ratio=10):
    ...
    def __call__(self, results):
        ...
        """
        copy_val = np.random.random() < self.copy_ratio
        if copy_val:
