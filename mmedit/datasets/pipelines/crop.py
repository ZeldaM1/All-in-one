...
@PIPELINES.register_module()
class TripleRandomCrop(PairedRandomCrop):
    def __call__(self, results):
        scale = results['scale']
        lq_patch_size = self.gt_patch_size // scale
        ...
        lq_is_list2 = isinstance(results['lq2'], list)
        if not lq_is_list2:
            results['lq2'] = [results['lq2']]
        ...
        h_lq, w_lq, _ = results['lq2'][0].shape

        if h_lq < lq_patch_size or w_lq < lq_patch_size:
            raise ValueError(
                f'LQ2 ({h_lq}, {w_lq}) is smaller than patch size '
                f'({lq_patch_size}, {lq_patch_size}). Please check '
                f'{results["lq_path"][0]} and {results["gt_path"][0]}.')
        top = np.random.randint(h_lq - lq_patch_size + 1)
        left = np.random.randint(w_lq - lq_patch_size + 1)
        ...
        results['lq2'] = [
            v[top:top + lq_patch_size, left:left + lq_patch_size, ...]
            for v in results['lq2']
        ]

        if not lq_is_list:
            results['lq'] = results['lq'][0]
        if not lq_is_list2:
            results['lq2'] = results['lq2'][0]
        if not gt_is_list:
            results['gt'] = results['gt'][0]
        return results
