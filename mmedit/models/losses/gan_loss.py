...
class GANLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if self.gan_type == 'vanilla':
            ...
            self.loss = nn.ReLU()
        elif self.gan_type == 'project':
            self.loss = self._projected_loss
        else:
    ...
    def _projected_loss(self, input_logits, input_type):
        if input_type=='G_gen':
            loss_Gmain = sum([(-l).mean() for l in input_logits])
            return loss_Gmain
            
            # gen_logits = torch.cat(gen_logits)
            # return 'Loss/scores/fake', gen_logits,'Loss/signs/fake', gen_logits.sign(),'Loss/G/loss', loss_Gmain
        
        elif input_type=='D_fake':
            loss_Dgen = sum([(F.relu(torch.ones_like(l) + l)).mean() for l in input_logits])
            return loss_Dgen
            # gen_logits = torch.cat(gen_logits)
            # training_stats.report('Loss/scores/fake', gen_logits)
            # training_stats.report('Loss/signs/fake', gen_logits.sign())
        
        elif input_type=='D_real':
            loss_Dreal = sum([(F.relu(torch.ones_like(l) - l)).mean() for l in input_logits])
            return loss_Dreal
    ...
    def forward(self, input, target_is_real=None, is_disc=False, mask=None, input_type=None):
        """
        Args:
        ...
        """
        if not (self.gan_type in ["project"]):
        ...
        if self.gan_type == 'hinge':
            ...
            loss = self.loss(input, input_type=input_type) if (self.gan_type in ["project"]) else self.loss(input, target_label)  
