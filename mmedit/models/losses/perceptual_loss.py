...
@LOSSES.register_module()
class PerceptualLoss_lambda(PerceptualLoss):
    def __init__(self, layer_weights, criterion='l1', **kwargs):
        super().__init__(layer_weights,**kwargs)
        criterion = criterion.lower()
        if criterion == 'l1':
    ...
    def forward(self, x, gt, lmbdas):
        ...
        if self.perceptual_weight > 0:
            percep_loss = 0
            for k in x_features.keys():
                pixel_loss = self.criterion(x_features[k], gt_features[k])
                pixel_loss = torch.mean(pixel_loss,dim=[1,2,3])
                percep_loss += torch.sum(pixel_loss * lmbdas)/int(len(lmbdas)) * self.layer_weights[k]
            percep_loss *= self.perceptual_weight
        else:
            percep_loss = None
