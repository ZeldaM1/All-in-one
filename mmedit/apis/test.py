...
from mmedit.utils import get_padding_size 
from mmedit.core import tensor2img
from pytorch_msssim import ms_ssim
import numpy as np
from mmedit.utils.functional import ycbcr444_to_420,ycbcr420_to_rgb,ycbcr444_to_rgb,ycbcr2rgb
import time
...
def multi_gpu_test(model,data_loader,tmpdir=None,
    ...
    for frame_idx, data in enumerate(data_loader):
        ...
        # pad for compute yuv420 PSNR
        
        if model.module.generator.module.yuv420:
            padding_l, padding_r, padding_t, padding_b = get_padding_size(pic_height, pic_width, 2)
            template = F.pad(x_hat,(padding_l, padding_r, padding_t, padding_b),mode="replicate",)
            x_hat = F.interpolate(x_hat, (template.shape[2],template.shape[3]), mode='bilinear', align_corners=True)
            gt = F.interpolate(gt, (template.shape[2],template.shape[3]), mode='bilinear', align_corners=True)
 
        psnr,y_rec,uv_rec  = PSNR(x_hat, gt.to(x_hat.device),model.module.generator.module.yuv420)


        
        # # x_hat, gt = ycbcr2rgb(x_hat), ycbcr2rgb(gt)
        # x_hat=x_hat.squeeze(0).cpu().numpy()
        # gt=gt.squeeze(0).cpu().numpy()


        # y_rec, uv_rec = ycbcr444_to_420(x_hat)
        # y_gt, uv_gt = ycbcr444_to_420(gt)
        # x_hat, gt = ycbcr420_to_rgb( y_rec, uv_rec), ycbcr420_to_rgb(y_gt, uv_gt)
        # psnr=calc_psnr(x_hat, gt, data_range=1)
        ...
        if "bit" in result.keys():
            bit_all.append(float(result["bit"].cpu()) / pic_height / pic_width)
        else:
            bit_all.append(0)
        pix_all.append(pic_height*pic_width)
        bpp=float(result["bpp"].cpu()) if "bpp" in result.keys() else 0
        ...
        if save_image:
            ...
            save_path_dir,img_name =meta[0]['lq_path'].split("/")[-2],meta[0]['lq_path'].split("/")[-1]
