...
class EvalIterHook(Hook):
    ...
    def after_train_epoch(self, runner):
        ...
        if not self.every_n_iters(runner, self.interval):
            return
        runner.log_buffer.clear()
        model_path=f'{runner.work_dir}/epoch_{int(runner.epoch)+1}.pth'
        import os
        command=f"./tools/dist_test.sh  {runner.config_file}   {model_path}  1  '--test_config  ./test_cfg/zhm_rgb.json    -w 4 --rate_num 4  \
        --float16 0 --is_rt 0 --ec_thread 0   --cal_msssim False  \
        --json_path   ./dmci_test.json'"
        os.system(command)
...
class DistEvalIterHook_iter(EvalIterHook):
    ...
    def after_train_iter(self, runner):
        ...
        model_path=f'{runner.work_dir}/iter_{int(runner.iter)+1}.pth'
        import os
        command=f"./tools/dist_test.sh  {runner.config_file}  {model_path}  1  '--test_dataset  {runner.test_dataset}   --test_config  ./test_cfg/zhm_rgb.json    -w 4 --rate_num 4  \
        --float16 0 --is_rt 0 --ec_thread 0   --cal_msssim False '"
